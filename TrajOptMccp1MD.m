classdef TrajOptMccp1MD
    % For Maccepavd with motor dynamics
    ...
    methods (Static)
        ...
        function [ traj ] = reachFastIlqr_fixu1( robot, x0, xf, varargin )
            ...
            costpara.w_tf = 1e3;
            costpara.w_t  = 1e3; %
            costpara.w_e  = sfactor*1e1; % 1 is good for spring force
            j = @(x,u,t)TrajOptMccp1MD.jReachFast(x, u, t, costpara);
            %%%
            opt_param = [];
        ...
        % x0, xf, tf, dt, m2min
            ...
            costpara.w_tf = 1e4;
            costpara.w_t  = 0; %
            costpara.w_e  = sfactor*1e0; % 1 is good for spring force
...
function c = costr_reachFast(x, u, t, para)
            ...
            %ce = (para.robot.spring_force(x,u)).^2 + 0.5*(u(3)-0.5)^2;
            %ce = (u(1) - para.qf)^2 + u(2)^2 + 0.5*(u(3)-0.5)^2;
            %ce = (u(1) - x(1))^2 + u(2)^2 + 0.5*(u(3)-0.5)^2;
            ce = (u(1) - x(1))^2 + u(2)^2 + 1e-3*(u(3)-0.5)^2;
            %ce = (u(1) - x(3))^2 + u(2)^2 + 1e-3*(u(3)-0.5)^2;
            %ce = (u(1) - x(3))^2 + (u(2) - x(4))^2 + 1e-3*(u(3)-0.5)^2;
...
c1 = (x(1) - para.qf).^2*B;
%ce = (para.robot.spring_force(x,u)).^2;
ce = (u(1) - para.qf)^2 + u(2)^2 + 1e-3*(u(3)-0.5)^2;
