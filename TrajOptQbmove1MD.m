classdef TrajOptQbmove1MD
    ...
    methods (Static)
        ...
        function [ traj ] = reachFastIlqr( robot, x0, xf, varargin )
            ...
            %end
            %if nargin < 6
            tension = parser.Results.tension;
            %end
            if isscalar(x0)
                x0 = [x0;0;x0+tension;x0-tension;0;0];
            end
            qf = xf(1);
            if isscalar(xf)
                Hf = diag([1, 0, 0, 0, 0, 0]);
            ...
            j = @(x,u,t)TrajOptQbmove1MD.jReachFast(x, u, t, costpara);
            %%%
            opt_param = [];
            ...
            opt_param.T = tf;
            %%%
            opt_param.umax = [pi; pi; 1];
            opt_param.umin = [-pi; -pi; 0];
            % u0 can be full command sequence or just initial point
            u0 = [qf+tension; qf-tension; 0.1];
            %u0 = [0; 0.1; 0];
        ...
        % x0, xf, tf, dt, m2min
            ...
            j = @(x,u,t)TrajOptQbmove1MD.jReachSmooth( x, u, t, costpara);
            %%%
            opt_param = [];
...
function c = costr_reachFast(x, u, t, para)
            ...
            ce = (u(1) - x(1))^2 + (u(2) - x(1))^2 + 0.1*(u(3)-0.5)^2;
...
c1 = (x(1) - para.qf).^2*B;
%ce = (para.robot.spring_force(x,u)).^2;
ce = (u(1) - x(1))^2 + (u(2)-x(1))^2 + 1e-6*u(3)^2;
%ce = (u(1) - x(3))^2 + (u(2) - x(4))^2 + 1e-6*u(3)^2;
